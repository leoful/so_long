...
/*   Updated: 2024/12/16 17:22:45 by lbard            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_parsing_map(t_game *game, char *map_file)
{
    ...
    if (fd < 0)
        return (1);
    game->map = malloc(sizeof(char *) * 200);  // Allocation initiale
    if (!game->map)
        return (1);
    while ((line = get_next_line(fd)))
    {
        game->map[i++] = line;
        game->count++;
        printf("Ligne %d: %s", i - 1, line);  // Afficher chaque ligne
    }
    game->map[i] = NULL;
    close(fd);
    return (0);
...
int contour_map(t_game *game)
{
    ...
    while (game->map[0][column])
    {
        printf("Vérification du bord supérieur, colonne %d: %c\n", column, game->map[0][column - 1]);
        if (game->map[0][column] != '1')
            return (1);
        column++;
    }
    column = 0;
    // verifier le bord bas de la map
    while (game->map[game->count - 1][column])
    {
        printf("Vérification du bord supérieur, colonne %d: %c\n", column, game->map[0][column]);
        if (game->map[game->count - 1][column] != '1')
            return (1);
        column++;
    }
    //verifier les bords gauche et droit de la map
    while (line < game->count)// pour ignorer la derniere ligne
    {
...
int in_map(t_game *game)
{
    ...
    count_p = 0;
    count_c = 0;
    // on parcours tout le tableau il doit y avoir un E et un P seulement 1
    while (game->map[line])
    {
        column = 0;
...
// verifier si il existe un chemin valide
